...
import LatesNewsSection from "../../views/latest-news";
import CompanyLatesNewsSection from "../../views/company-latest-news";
...
export default function CompanyArticles({ sortBy }: { sortBy: string }) {
  ...
  // else if (data.length === 0 || !data[0]?.attributes) return <>no found</>

  return (
    <div>
      {isLoading ? <div className="w-full grid grid-cols-4 gap-5 md:mt-6 mt-[16px]" >
        ...
        <CompanyLatesNewsSection data={data} />}
    </div>
  );
}
