...
app.post('/', async (req, res) => {
    console.log("reached")
    if (req.headers.authorization !== process.env.REACT_APP_PUBLIC_WEBHOOK_TOKEN) {
        console.log("invalid")
        res.status(403).send({
            status: 403,
        ...
        generate(process.env.REACT_APP_PUBLIC_FRONTEND_HOST, 'sitemaps', './public',
            ['/api/', '/login'])
            .then(() => {
...
app.listen(process.env.REACT_APP_PUBLIC_SITEMAP_PORT, () => {
    console.log(`Sitemap Generator at http://localhost:${process.env.REACT_APP_PUBLIC_SITEMAP_PORT}`)
})
